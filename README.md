...
Die App nutzt Umgebungsvariablen aus der .env mit Defaults.
Setze bei Bedarf:
...
Für Schulen/Datenschutz: Alle Ziele sind hart auf 127.0.0.1 beschränkt.
Wenn du ein anderes Audio-Device nutzen willst, setze APP_AUDIO_DEVICE auf den genauen Namen (pactl list sources short hilft).

#### Start

Firefox einrichten

Um beim Entwickeln nicht bei jedem Refresh klicken zu müssen, können Sie die Abfrage in Firefox temporär global deaktivieren.

Geben Sie

	about:config

in die Adressleiste ein und bestätigen Sie die Warnung.
Suchen Sie nach
...
Hinweis: Dies erlaubt allen Webseiten den Zugriff ohne Nachfrage. Stellen Sie den Wert nach der Entwicklung wieder auf false.
...
Alle ~7 s wird ein Transkript-Snapshot erzeugt:
...
Optional: Ollama einrichten (LLM)
...
pywhispercpp integrieren (Produktiv-Transkription).
