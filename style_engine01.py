...
class StyleEnv(BaseModel):
    ollama_host: str = Field(default=os.getenv("APP_OLLAMA_HOST", os.getenv("OLLAMA_HOST", "http://127.0.0.1:11434")))
    ollama_vision_model: str = Field(default=os.getenv("APP_OLLAMA_VISION_MODEL", os.getenv("OLLAMA_VISION_MODEL", "")))
    vision_enabled: bool = Field(default=os.getenv("FEATURE_STYLE_REF_VISION_CLOUD", "1") == "1")
    timeout: float = Field(default=float(os.getenv("APP_HTTP_TIMEOUT", "30.0")))
...
def log_style_env() -> None:
    # Keep logs safe: don't print secrets, only presence of values
    logger.info(
        "[STYLE] vision_enabled=%s ollama_host=%s vision_model_set=%s",
        ENV.vision_enabled,
        ENV.ollama_host,
        bool(ENV.ollama_vision_model),
    )
...
def _env_bool01(name: str, default01: int = 0) -> bool:
    """Read boolean from env accepting 1/0, true/false, yes/no."""
    v = os.getenv(name)
    if v is None:
        return bool(default01)
    v = v.strip().lower()
...
def _debug() -> bool:
    """Global debug flag for verbose prints."""
    return _env_bool01("APP_DEBUG", 0)

async def _ollama_chat_vision(
    client: "GuardedAsyncClient",  # expect guarded client injection
    image_bytes: bytes,
    prompt: str,
) -> str:
    """
    Send a single-turn vision prompt to Ollama /api/chat with an inline base64 image.
    - Uses is_local_llm=True to pass NetGuard.
    - Low temperature for stable descriptors.
    """
    if not ENV.vision_enabled:
        raise RuntimeError("Vision features are disabled by config.")
    if not ENV.ollama_vision_model:
        raise RuntimeError("No vision model configured (APP_OLLAMA_VISION_MODEL).")

    # Prepare chat messages with base64 image
    ...
    url = f"{ENV.ollama_host.rstrip('/')}/api/chat"

    # Minimal retry, leveraging outer client timeouts
    backoff = 0.5
    for attempt in range(3):
        try:
            r = await client.request(
                "POST",
                url,
                is_local_llm=True,  # critical: mark as local LLM
            ...
            if r.status_code < 400:
                ...
                return "\n".join(parts).strip()
            logger.warning(f"[STYLE] Ollama vision {r.status_code}: {r.text}")
        except PermissionError as pe:
            logger.error(f"[STYLE] NetGuard blocked vision call: {pe}")
            raise
        except Exception as e:
            logger.warning(f"[STYLE] Vision attempt {attempt+1} failed: {e}")
        await asyncio.sleep(backoff)
        backoff = min(2.0, backoff * 2)

    raise RuntimeError("Vision descriptor generation failed after retries.")


# Public API of style engine (example)
class StyleEngine:
    def __init__(self, client: "GuardedAsyncClient"):
        self.client = client
        log_style_env()

    async def build_style_descriptor(self, image_bytes: bytes) -> str:
        """
        Builds a normalized textual descriptor from a style reference image using local Ollama VLM.
        """
        prompt = (
            "Describe the visual style succinctly (tone, palette, composition, texture). "
            "Return a compact bullet-like summary, no more than 60 words."
        )
        desc = await _ollama_chat_vision(self.client, image_bytes, prompt)
...
def _log_style_env() -> None:
    """Print key style-related environment configuration for diagnostics (consolidated)."""
    try:
        print(
            "[STYLE-ENV]",
            ...
            f"ollama_vision_model={ENV.ollama_vision_model or '(unset)'}",
        )
    except Exception as e:
        print(f"[STYLE-ENV] log failed: {e}")
