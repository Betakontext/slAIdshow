# image_backend.py
# Comments strictly in English; short German notes for complex blocks.
...
def _env_str(k: str, d: str = "") -> str:
    # English: Return trimmed env string (or default)
...
def _clamp8(v: int) -> int:
    # English: Align dimension to multiple of 8 to match latent constraints
    v = max(64, min(4096, int(v)))
    return v - (v % 8)

def _size_from_wh(width: int, height: int) -> str:
    # English: Pollinations v1 expects "WxH"
...
class PollinationsConfig(BaseModel):
    ...
    def require_cloud_enabled(self) -> None:
        ...
        if not self.secret and self.use_v1:
            # Streng: v1 braucht Secret. Wenn du GET-Only ohne Secret erlauben willst, setze use_v1=0.
            raise RuntimeError("POLLINATIONS_SECRET missing for v1 usage (set POLLINATIONS_USE_V1=0 to allow GET flavor)")
...
class PollinationsBackend(ImageBackend):
    ...
    - Optional style descriptors and negative prompts supported
    ...
    async def _fetch_get(self, prompt: str, width: int | None, height: int | None) -> Path:
        ...
        # Hinweis: Strenge Policy (allow_cloud/secret) wird in generate() geprüft. GET kann optional key in params nutzen.
        w = width if (width and width > 0) else self.cfg.width
        h = height if (height and height > 0) else self.cfg.height
    ...
    async def generate(
        ...
        **kwargs: Any,
        ...
        # English: Strictly stay on Pollinations code path; no Comfy usage here.
        # Cloud + Secret policy (v1) check:
        if self.cfg.use_v1:
            self.cfg.require_cloud_enabled()
        else:
            # If not using v1, still require allow_cloud to be explicit.
            if not self.cfg.allow_cloud:
                raise RuntimeError("Cloud image backend not allowed (set ALLOW_CLOUD_IMAGE_BACKEND=1)")

        # Optional style descriptors from style_engine (prefer cloud descriptors to match Ref impl)
        ...
        if isinstance(style_reference_path, (str, Path)) and str(style_reference_path).strip():
            ...
            if p.exists():
                # Resolve descriptors similarly to reference (prefer_cloud=True)
                try:
                    desc = resolve_style_descriptors_for_reference(ref_path=p, prefer_cloud=True)  # type: ignore[arg-type]
                    if desc:
                        style_descriptors = (style_descriptors or []) + desc
                except Exception:
                    pass
        ...
        # Deutsch: negative Prompt als Suffix-Zeile (Kompatibilität)
        ...
        if isinstance(style_reference_path, (str, Path)) and str(style_reference_path).strip():
            ...
            if p.exists() and p.is_file():
                ...
                return await self._post_v1_edits_multipart(full_prompt, p, eff_w, eff_h, negative_prompt=n_prompt or None, seed=seed_override)
...
class ComfyConfigBase(BaseModel):
    ...
    def assert_host_policy(self) -> None:
        # English: Hook reserved for host/whitelist policy; kept as no-op per user's relaxed policy
        return
...
class LocalComfyBackend(ImageBackend):
    ...
    def __init__(self, out_dir: Path, cfg: ComfyConfigBase, style: Optional[StyleRuntime] = None) -> None:
        ...
        # English: Lazy-import the bridge only here so Pollinations can run without comfy deps installed
        self._bridge_generate = None
        self._bridge_url_patch_fn = None
        self._has_url_patch = False
        self._lazy_import_bridge()

    def _lazy_import_bridge(self) -> None:
        """Import Comfy bridge functions lazily to avoid side-effects for non-Comfy backends."""
        try:
            from comfyui_bridge import generate_from_prompt_dict  # type: ignore
            self._bridge_generate = generate_from_prompt_dict
        except Exception as e:
            raise ImportError(f"Missing required bridge: comfyui_bridge.generate_from_prompt_dict ({e})") from e

        # Optional URL-mode helper: try comfy_bridge first, then comfyui_bridge
        try:
            from comfy_bridge import stage_reference_url_and_patch_prompt_sync as _url_patch  # type: ignore
            self._bridge_url_patch_fn = _url_patch
            self._has_url_patch = True
        except Exception:
            try:
                from comfyui_bridge import stage_reference_url_and_patch_prompt_sync as _url_patch_alt  # type: ignore
                self._bridge_url_patch_fn = _url_patch_alt
                self._has_url_patch = True
            except Exception:
                self._bridge_url_patch_fn = None
                self._has_url_patch = False
    ...
    def _copy_to_comfy_input(self, src: Path) -> Path:
        ...
        try:
            ...
            dst = inp / src.name  # sanitize basename
    ...
    async def generate(self, prompt: str, width: int | None = None, height: int | None = None, negative_prompt: str | None = None, **kwargs: Any) -> Path:
        ...
        if self.style and self.style.has_reference:
            ...
            if ref_mode == "url" and self._has_url_patch and callable(self._bridge_url_patch_fn):
                try:
                    # Wrap into {'prompt': {...}} for the helper and unwrap back
                    body: Dict[str, Any] = {"prompt": prompt_dict}
                    body = self._bridge_url_patch_fn(  # type: ignore[misc]
                        prompt_dict=body,
        ...
        # Dispatch to bridge
        ts = _now()
        images: List[Path] = await self._bridge_generate(  # type: ignore[misc]
...
class BackendEnv(BaseModel):
    # Accepts: comfyui|comfyui_local|comfyui_remote|pollinations
    # Priority: APP_IMAGE_BACKEND (if set) else IMAGE_BACKEND
    image_backend: str = Field(default_factory=lambda: (_env_str("APP_IMAGE_BACKEND", "") or _env_str("IMAGE_BACKEND", "comfyui")).lower())
    allow_cloud: bool = Field(default_factory=lambda: _env_bool01("ALLOW_CLOUD_IMAGE_BACKEND", 0))
...
def build_image_backend(style: Optional[StyleRuntime] = None) -> ImageBackend:
    ...
    # English: Diagnostic log to see exact selection
    if _debug():
        print(f"[IMG][factory] selected backend='{backend}' allow_cloud={env.allow_cloud} out_dir='{out_dir}'")

    # Normalize aliases so 'comfyui' means local unless explicitly 'comfyui_remote'
    ...
    if backend == "pollinations":
        ...
        cfg.allow_cloud = bool(env.allow_cloud)  # propagate allow flag
