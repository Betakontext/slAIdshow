...
async def audio_transcription_loop() -> None:
    ...
    if not audio_frames:
        ...
        try:
            ...
            print(f"[AUDIO] retry blocksize=0 failed: {e}")
    ...
    try:
        while STATE.running:
            ...
            if now - last_tick >= 1.0:
                last_tick = now
                await broadcast("status", f"tick frames={total_frames} seg_frames={len(current_segment_frames)} sr={actual_sr}")
            ...
            total_frames += 1
            target = int(actual_sr * FRAME_MS / 1000)
            if len(frame) != target:
                if len(frame) < target:
                    frame = np.pad(frame, (0, target - len(frame)))
                else:
                    frame = frame[:target]
            frame = np.clip(frame, -1.0, 1.0)

            is_speech = True if DISABLE_VAD else rms_vad(frame, rms_threshold=RMS_VAD_THRESHOLD)
...
async def selftest_callback():
    ...
    try:
        ...
        def cb(indata, frames, timeinfo, status):
            nonlocal got
            if status:
                print(f"[SELFTEST] callback status: {status}")
            got += 1
        stream = sd.InputStream(samplerate=sr, channels=1, dtype="float32", device=dev_param, blocksize=frame_samples, callback=cb, latency="low")
        stream.start()
        t0 = time.time()
        while time.time() - t0 < 1.0 and got == 0:
            await asyncio.sleep(0.05)
        with contextlib.suppress(Exception):
            stream.stop(); stream.close()
        return {"device": name, "dev_param": str(dev_param), "samplerate": sr, "blocksize": frame_samples, "callback_frames": got}
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500)
    ...
    try:
        async with httpx.AsyncClient() as client:
            body = {
                ...
                "prompt": "Say hello.",
                "stream": False,
                "options": {"temperature": OLLAMA_TEMPERATURE, "num_ctx": max(512, min(OLLAMA_NUM_CTX, 2048)), "num_predict": 8},
            }
